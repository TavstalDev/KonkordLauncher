...
public class QuiltInstaller : MinecraftInstaller
{
    ...
    private readonly string _quiltManifestPath;
    ...
    public QuiltInstaller(string javaPath, string minecraftVersion, int memory, LauncherDetails launcherDetails, ClientDetails clientDetails, 
        string assetsDir, string cacheDir, string libsDir, string manifestPath, string versionsDir, string quiltManifestPath,
    ...
    {
        _quiltManifestPath = quiltManifestPath;
    }
    ...
    protected override async Task<ModdedData?> InstallModedAsync(string tempDir)
    {
        ...
        if (!File.Exists(_quiltManifestPath))
        {
            _logger.Error("Quilt manifest file does not exist. Please ensure the manifest is downloaded.");
            return null;
        }

        VersionDetails quiltVersion = GameHelper.GetVersionDetails(VersionsDir, this.MinecraftVersion.Id, EMinecraftKind.QUILT, this.VersionData.CustomVersion, this.VersionData.GameDir);
        ...
        // Download Loader
        string loaderDirPath = Path.Combine(LibsDir, $"net\\fabricmc\\fabric-loader\\{quiltVersion.CustomVersion}");
