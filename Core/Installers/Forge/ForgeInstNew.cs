...
public class ForgeInstNew : ForgeInstallerBase
{
    ...
    public ForgeInstNew(string javaPath, string minecraftVersion, int memory, LauncherDetails launcherDetails, ClientDetails clientDetails, 
        ...
        : base(javaPath, minecraftVersion, memory, launcherDetails, clientDetails, assetsDir, cacheDir, libsDir, manifestPath, versionsDir, forgeManifestPath, kind, gameDirectory, resolution, jvmArgs, customVersion, progressReporter, isDebug)
    {
    }
    ...
    protected override async Task<ModdedData?> InstallModedAsync(string tempDir)
    {
        ...
        if (!File.Exists(ForgeManifestPath))
        {
            _logger.Error("Forge manifest file does not exist. Please ensure the manifest is downloaded.");
            return null;
        }

        VersionDetails forgeVersion = GameHelper.GetVersionDetails(VersionsDir, this.MinecraftVersion.Id, EMinecraftKind.FORGE, this.VersionData.CustomVersion, this.VersionData.GameDir);
        ...
        // Check libsizes dir
        string librarySizeCacheDir = Path.Combine(CacheDir, "libsizes");
        ...
        if (Directory.Exists(mavenTempDir))
        {
            ...
            {
                string newPath = file.Replace(mavenTempDir, LibsDir);
        ...
        foreach (LibraryMeta lib in installProfile.Libraries)
        {
            ...
            string libDirPath = Path.Combine(LibsDir, localPath.Remove(localPath.LastIndexOf('/'), localPath.Length - localPath.LastIndexOf('/')));
        ...
        string forgeUniversalDir = Path.Combine(LibsDir, $"net\\minecraftforge\\forge\\{forgeVersion.MinecraftVersion}-{forgeVersion.CustomVersion}");
