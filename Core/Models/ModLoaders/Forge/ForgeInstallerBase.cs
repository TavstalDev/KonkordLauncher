...
public abstract class ForgeInstallerBase : MinecraftInstaller
{
    ...
    protected string ForgeManifestPath { get; set; }
    ...
    /// <summary>
    /// Maps processor data and starts the processors for the Forge installation.
    /// </summary>
    /// <param name="installProfile">The Forge version profile containing installation data.</param>
    /// <param name="installerDir">The directory where the installer files are located.</param>
    ...
    /// <summary>
    /// Maps processor data for the Forge installation.
    /// </summary>
    /// <param name="data">The processor data in JSON format.</param>
    /// <param name="kind">The type of installation (e.g., client).</param>
    /// <param name="minecraftJar">The path to the Minecraft JAR file.</param>
    /// <param name="installDir">The directory where the installation files are located.</param>
    ...
    protected Dictionary<string, string?> MapProcessorData(JObject data, string kind, string minecraftJar, string installDir)
    {
        ...
        foreach (KeyValuePair<string, JToken?> item in data)
        {
            ...
            string? fullPath = Mapper.ToFullPath(value, LibsDir);
            if (fullPath == value)
            {
                value = value.Trim('/');
    ...
    /// </summary>
    /// <param name="processors">The list of processors to execute.</param>
    ...
    private async Task startProcessor(JToken processor, Dictionary<string, string?> mapData)
    {
        ...
        string jarPath = Path.Combine(LibsDir, jar.GetPath());
        ...
        if (classpathObj != null)
        {
            foreach (var libName in classpathObj)
            {
                ...
                string? lib = Path.Combine(LibsDir,
        ...
        if (argsArr != null)
        {
            ...
            args = Mapper.Map(arrStrs, mapData, LibsDir);
        }
