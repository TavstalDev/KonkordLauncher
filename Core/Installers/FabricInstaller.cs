...
public class FabricInstaller : MinecraftInstaller
{
    ...
    private readonly string _fabricManifestPath;
    ...
    public FabricInstaller(string javaPath, string minecraftVersion, int memory, LauncherDetails launcherDetails, ClientDetails clientDetails, 
        string assetsDir, string cacheDir, string libsDir, string manifestPath, string versionsDir, string fabricManifestPath,
    ...
    {
        _fabricManifestPath = fabricManifestPath;
    }
    ...
    protected override async Task<ModdedData?> InstallModedAsync(string tempDir)
    {
        ...
        if (!File.Exists(_fabricManifestPath))
        {
            _logger.Error("Fabric manifest file not found at path: " + _fabricManifestPath);
            return null;
        }

        VersionDetails fabricVersion = GameHelper.GetVersionDetails(VersionsDir, this.MinecraftVersion.Id, EMinecraftKind.FABRIC, this.VersionData.CustomVersion, this.VersionData.GameDir);
        ...
        // Download Loader
        string loaderDirPath = Path.Combine(LibsDir, $"net\\fabricmc\\fabric-loader\\{fabricVersion.CustomVersion}");
